...
export class ZardInputGroupComponent {
  @ContentChild(ZardInputDirective, { static: false }) inputDirective?: ZardInputDirective;
  ...
  private readonly uniqueId = Math.random().toString(36).substr(2, 9);
  protected readonly addonBeforeId = computed(() => `input-group-addon-before-${this.uniqueId}`);
  protected readonly addonAfterId = computed(() => `input-group-addon-after-${this.uniqueId}`);
  protected readonly prefixId = computed(() => `input-group-prefix-${this.uniqueId}`);
  protected readonly suffixId = computed(() => `input-group-suffix-${this.uniqueId}`);
