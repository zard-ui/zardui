  ...
  template: `
    <div class="flex flex-col gap-6 sm:gap-8">
      <p class="text-muted-foreground text-base leading-relaxed sm:text-lg">
        The <code class="bg-muted rounded px-1.5 py-0.5 text-xs sm:text-sm">components.json</code> file holds
      ...
      <z-callout title="Note: The components.json file is optional" icon="ℹ" variant="info">
        It is <strong>only required if you're using the CLI</strong> to add components to your project. If you're using
        the copy and paste method, you don't need this file.
      </z-callout>

      <div class="flex flex-col gap-4">
        <p class="text-muted-foreground text-sm sm:text-base">
          You can create a <code class="bg-muted rounded px-1.5 py-0.5 text-xs sm:text-sm">components.json</code> file
