    ...
    <section class="flex flex-col gap-6 sm:gap-8" scrollSpyItem="package-manager" id="package-manager">
      <div class="flex flex-col gap-4 sm:gap-6">
        ...
        <p class="text-muted-foreground text-base leading-relaxed sm:text-lg">
          The package manager to use for installing dependencies. This configuration helps the CLI use the correct
          package runner commands for your project.
        </p>
        <p class="text-muted-foreground text-base leading-relaxed sm:text-lg">
          <strong>Supported values:</strong>
          <code class="bg-muted relative rounded px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold">npm</code>
          ,
          <code class="bg-muted relative rounded px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold">pnpm</code>
          ,
          <code class="bg-muted relative rounded px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold">yarn</code>
          , or
          <code class="bg-muted relative rounded px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold">bun</code>
        </p>
        <z-code-block [data]="packageManagerExample" />
      </div>
    </section>
  `,
})
export class JsonPackageManagerSectionComponent {
  readonly packageManagerExample: CodeBlockData = JSON_PACKAGE_MANAGER_EXAMPLE;
}
