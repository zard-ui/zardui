...
@Component({
  selector: 'z-sidebar',
  template: `
    <aside class="sticky top-20 z-30 w-[var(--sidebar-width)] max-h-[calc(100vh-5rem)] bg-transparent hidden flex-col lg:flex shrink-0">
      <nav class="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto no-scrollbar pb-12">
        @for (doc of sidebarPaths; track $index) {
          <div class="relative flex w-full min-w-0 flex-col p-2">
            <ul class="flex w-full min-w-0 flex-col gap-0.5">
              <h1 class="flex shrink-0 items-center h-8 px-2 text-xs outline-hidden text-muted-foreground font-medium">{{ doc.title }}</h1>
              @for (path of doc.data; track $index) {
                <li class="relative">
                  <a
                    ...
                    class="justify-between px-2 py-1 h-8 font-normal capitalize max-w-48"
