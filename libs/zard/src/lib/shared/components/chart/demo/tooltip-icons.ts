...
@Component({
  ...
  template: `
    <z-card class="w-full">
      <z-card-header>
        <z-card-title zTitle="Tooltip - Icons" />
        <z-card-description zDescription="Tooltip with icons." />
      </z-card-header>
      <z-card-content>
        <z-chart
          zType="bar"
          ...
          class="w-full"
        >
          <z-chart-tooltip zHideLabel [zDefaultIndex]="1" />
          <z-chart-legend />
        </z-chart>
      </z-card-content>
    </z-card>
...
export class ZardDemoChartTooltipIconsComponent {
  protected readonly chartConfig: ZardChartConfig = {
    running: { label: 'Running', color: 'var(--chart-1)', icon: 'lucideFootprints' },
    swimming: { label: 'Swimming', color: 'var(--chart-2)', icon: 'lucideWaves' },
  };
