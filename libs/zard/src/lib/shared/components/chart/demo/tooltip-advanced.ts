...
@Component({
  ...
  template: `
    <z-card class="w-full">
      <z-card-header>
        <z-card-title zTitle="Tooltip - Advanced" />
        <z-card-description zDescription="Tooltip with custom formatter and total." />
      </z-card-header>
      <z-card-content>
        <z-chart
          zType="bar"
          ...
          class="w-full"
        >
          <z-chart-tooltip zHideLabel class="w-[180px]" [zValueFormatter]="kcal" [zDefaultIndex]="1" />
        </z-chart>
      </z-card-content>
    </z-card>
  `,
...
export class ZardDemoChartTooltipAdvancedComponent {
  ...
  protected readonly kcal = (value: number) => `${value.toLocaleString()} kcal`;
}
