...
@Component({
  ...
  template: `
    <z-card class="w-full">
      <z-card-header>
        <z-card-title zTitle="Radar Chart - Grid Filled" />
...
export class ZardDemoChartRadarGridFillComponent {
  ...
  protected readonly series: ZardChartSeries[] = [{ dataKey: 'desktop', fillOpacity: 0.5 }];

  /**
   * Tints the whole grid, the way shadcn paints `PolarGrid` with the series color at 20%.
   * `splitArea.areaStyle.opacity` is a zrender style ECharts' own types leave out.
   */
  protected readonly filledGrid = {
    radar: { splitArea: { show: true, areaStyle: { color: 'var(--chart-1)', opacity: 0.2 } } },
