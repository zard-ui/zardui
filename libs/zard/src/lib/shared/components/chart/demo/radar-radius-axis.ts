...
@Component({
  ...
  template: `
    <z-card class="w-full">
      <z-card-header>
        <z-card-title zTitle="Radar Chart - Radius Axis" />
...
export class ZardDemoChartRadarRadiusAxisComponent {
  ...
  /**
   * ECharts escape hatch: print the radius scale ECharts hides by default.
   *
   * Recharts draws `<PolarRadiusAxis>` on a single spoke; ECharts has no per-spoke option and
   * repeats the scale on every axis, so `splitNumber` is lowered to keep it readable.
   */
  protected readonly radiusAxis: ZardChartOptionOverride = {
    radar: {
      splitNumber: 3,
      axisLabel: { show: true, showMinLabel: false, color: 'var(--muted-foreground)', fontSize: 9 },
    },
  };
}
