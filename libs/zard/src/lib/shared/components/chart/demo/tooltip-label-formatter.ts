...
@Component({
  ...
  template: `
    <z-card class="w-full">
      <z-card-header>
        <z-card-title zTitle="Tooltip - Label Formatter" />
        <z-card-description zDescription="Tooltip with label formatter." />
      </z-card-header>
      <z-card-content>
        <z-chart
          zType="bar"
          ...
          class="w-full"
        >
          <z-chart-tooltip [zLabelFormatter]="longDate" [zDefaultIndex]="1" />
        </z-chart>
      </z-card-content>
    </z-card>
  `,
...
export class ZardDemoChartTooltipLabelFormatterComponent {
