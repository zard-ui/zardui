...
describe('ZardSwitchComponent', () => {
  ...
  describe('Label Association', () => {
    ...
    it('renders empty label when no content is provided via ng-content', async () => {
      const checked = signal(true);
      await render(ZardSwitchComponent, {
        bindings: [inputBinding('zChecked', checked)],
      });
      ...
      // Label should be present but have no text content
  ...
  describe('Reactive Forms', () => {
    let r: Awaited<ReturnType<typeof render<TestReactiveFormsModule>>>;

    beforeEach(async () => {
      r = await render(TestReactiveFormsModule, {
        providers: [
          {
  ...
  describe('Thumb Element', () => {
    it('renders thumb element with correct classes', async () => {
      const checked = signal(true);
      await render(ZardSwitchComponent, {
        bindings: [inputBinding('zChecked', checked)],
      });
    ...
    it('applies correct transform based on checked state', async () => {
      const checked = signal(true);
      const zSize = signal<'default' | 'sm' | 'lg'>('default');
    ...
    it('applies correct transform for small size and checked state', async () => {
      const checkedSignal = signal(true);
      const zSize = signal<'default' | 'sm' | 'lg'>('sm');

      const r = await render(ZardSwitchComponent, {
        bindings: [inputBinding('zChecked', checkedSignal), inputBinding('zSize', zSize)],
      });

      const switchButton = screen.getByRole('switch');
      const thumb = switchButton.querySelector('span');

      expect(thumb).toHaveClass('data-[size=sm]:data-[state=checked]:translate-x-4');
    });

    it('applies correct transform for default size and checked state', async () => {
      const checkedSignal = signal(true);
      const zSize = signal<'default' | 'sm' | 'lg'>('default');
    ...
    it('applies correct transform for large size and checked state', async () => {
      const checkedSignal = signal(true);
      const zSize = signal<'default' | 'sm' | 'lg'>('lg');
      ...
      expect(thumb).toHaveClass('data-[size=lg]:data-[state=checked]:translate-x-6');
    });

    it('applies correct unchecked transform for small size', async () => {
    ...
    it('applies correct unchecked transform for default size', async () => {
      const checkedSignal = signal(false);
      const zSize = signal<'default' | 'sm' | 'lg'>('default');
    ...
    it('applies correct unchecked transform for large size', async () => {
      const checkedSignal = signal(false);
      const zSize = signal<'default' | 'sm' | 'lg'>('lg');
    ...
    it('applies correct size attributes for small size', async () => {
      const checked = signal(true);
      const zSize = signal<'default' | 'sm' | 'lg'>('sm');
    ...
    it('applies correct size attributes for default size', async () => {
      const checked = signal(true);
      const zSize = signal<'default' | 'sm' | 'lg'>('default');
      ...
      expect(thumb).toHaveAttribute('data-size', 'default');
    });

    it('applies correct size attributes for large size', async () => {
      const checked = signal(true);
      const zSize = signal<'default' | 'sm' | 'lg'>('lg');
